...
import com.mcbproperty.authservice.exceptions.InvalidPermissionDataException;
import com.mcbproperty.authservice.exceptions.PermissionInUseException;
import com.mcbproperty.authservice.exceptions.PermissionNotFoundException;
...
public class PermissionService {
    ...
    public Iterable<Permission> getPermissionList(String organizationCraterId) {
        return permissionRepository.findAll();
    }

    public Permission getPermissionByKey(String organizationCraterId, String permissionKey) {
        if (Strings.isNullOrEmpty(permissionKey)) {
            throw new InvalidPermissionDataException("Permission key cannot be null or empty");
        }
        if (Strings.isNullOrEmpty(organizationCraterId)){
            throw new InvalidPermissionDataException("Organization Id key cannot be null or empty");
        }
        Optional<Permission> userOpt = permissionRepository.findByPermission(permissionKey);
        if (userOpt.isPresent()) {
            return userOpt.get();
        }
        throw new PermissionNotFoundException(String.format("Permission not found for permission key = %s", permissionKey));
    }
    ...
    public Permission createPermission(PermissionDTO permissionDTO) {
        ...
        String organizationId = permissionDTO.getOrganizationCraterId();
        ...
        permission.setOrganizationCraterId(permissionDTO.getOrganizationCraterId());
    ...
    @Transactional
    public void deletePermissionByKey(String organizationCraterId, String permissionKey) {
        if (Strings.isNullOrEmpty(permissionKey)) {
            throw new InvalidPermissionDataException("Permission key cannot be null or empty");
        }
